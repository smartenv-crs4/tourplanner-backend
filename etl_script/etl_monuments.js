...
var db = pmongo('156.148.14.146:3996/crawler', ['monumenti']);
  ...
  function insert_data(str_data)
{
    ...
    db.monumenti.find({}).toArray().then(function(docs){
...
function data_insert(docs)
 {
     ...
     str_query = "insert into dat_monument (fk_category, fk_city, _id, data, rating, time_to_visit) "
     + "values ($1, "
     + "$2, "
     + "$3, "
     + "$4, "
     ...
     sql.query(str_query, {bind: [1,1, docs._id, data, 6, 40], type: sql.QueryTypes.INSERT})
 ...
 {
     str_query = "select count(*) num  from dat_monument whwre id = $1 ";
